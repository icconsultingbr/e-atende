...
export class ReciboReceitaImpressaoService extends RelatorioService {
  constructor(private receitaService: ReceitaService, private pacienteLinkTemporatioService: PacienteLinkTemporarioService) {
    super();
  ...
  externalImprimir(ano: number, idEstabelecimento: number, numero: number, farmacia: boolean, target: string = '_blank') {
    this.pacienteLinkTemporatioService.obterRelatorio(ano, idEstabelecimento, numero)
      .subscribe((result) => {
        let cabecalhoMedicamento = '';
        const dadosEstoque = '';
        let dadosAssinatura = '';
